...
import { Sparkles, ShieldCheck, User, TrendingUp, CheckCircle2, Zap } from "lucide-react";

export default function ResumeHologramAnimation() {
  // Real-time Candidate Profile Cycling State
  const profiles = [
    {
      name: "Alex Vance",
  ...
  // Live Animated Counter States
  const [liveScore, setLiveScore] = useState(84);
  const [liveViews, setLiveViews] = useState(120);
  const [liveCalls, setLiveCalls] = useState(80);
  ...
  // Smooth Live Number Ticker Effect
  useEffect(() => {
    let frame: number;
    const duration = 1200;
    ...
    const initialViews = 90;
    const initialCalls = 50;

    const animateValues = (now: number) => {
      const elapsed = now - start;
      const progress = Math.min(elapsed / duration, 1);
      const easeOut = 1 - Math.pow(1 - progress, 3);

      setLiveScore(Math.floor(initialScore + (activeProfile.score - initialScore) * easeOut));
      setLiveViews(Math.floor(initialViews + (activeProfile.views - initialViews) * easeOut));
      setLiveCalls(Math.floor(initialCalls + (activeProfile.calls - initialCalls) * easeOut));

      if (progress < 1) {
        frame = requestAnimationFrame(animateValues);
      }
    };
  ...
  return (
    <div className="relative w-full max-w-[440px] mx-auto select-none pt-4 sm:pt-0">
      {/* Outer Constellation Orbit Rings */}
      ...
      {/* Real-time Floating Top Left Widget: Profile Score (Desktop Only) */}
      ...
      {/* Real-time Middle Right Widget: Recruiter Views (Desktop Only) */}
      <div className="hidden sm:block absolute top-1/3 -right-6 z-30 glass-card rounded-2xl p-3.5 border border-cgp-cyan/40 shadow-2xl backdrop-blur-xl bg-cgp-card/95 space-y-1">
        ...
        <span className="text-2xl font-black text-cgp-cyan block transition-all">
          +{liveViews}%
        </span>
      ...
      {/* Real-time Bottom Right Widget: Interview Calls (Desktop Only) */}
      <div className="hidden sm:block absolute -bottom-4 -right-4 z-30 glass-card rounded-2xl p-3.5 border border-cgp-pink/40 shadow-2xl backdrop-blur-xl bg-cgp-card/95 space-y-1">
        ...
        <span className="text-2xl font-black text-cgp-pink block transition-all">
          +{liveCalls}%
        </span>
        {/* Wave Sparkline */}
      ...
      <div className="relative z-10 glass-card rounded-3xl p-5 sm:p-6 border border-cgp-cyan/40 shadow-2xl bg-cgp-card/90 space-y-5 overflow-hidden">
        {/* Laser Scanner Beam Scanning Vertically */}
        ...
        <div className="sm:hidden grid grid-cols-3 gap-2 pb-3 border-b border-slate-800 text-center relative z-10">
          <div className="p-2 rounded-xl bg-cgp-surface/80 border border-cgp-purple/30">
            ...
            <span className="text-sm font-black text-emerald-400">{liveScore}/100</span>
          </div>
          <div className="p-2 rounded-xl bg-cgp-surface/80 border border-cgp-cyan/30">
            ...
            <span className="text-sm font-black text-cgp-cyan">+{liveViews}%</span>
          </div>
          <div className="p-2 rounded-xl bg-cgp-surface/80 border border-cgp-pink/30">
            ...
            <span className="text-sm font-black text-cgp-pink">+{liveCalls}%</span>
          </div>
        </div>

        {/* Top Header: Candidate Avatar & Real-time Cycling Profile Info */}
        <div
          className={`flex items-center gap-3.5 relative z-10 pb-4 border-b border-slate-800 transition-opacity duration-300 ${
        ...
        {/* Real-time Dynamic Glowing Metric Progress Bars */}
        <div
        ...
        >
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-extrabold text-cgp-cyan">{activeProfile.seoMatch}% Match</span>
            </div>
            <div className="w-full h-2 rounded-full bg-cgp-surface overflow-hidden p-0.5 border border-cgp-cyan/20">
              <div
                className="h-full rounded-full bg-gradient-to-r from-cgp-cyan to-cgp-blue shadow-[0_0_12px_#38bdf8] transition-all duration-1000 ease-out"
                style={{ width: `${activeProfile.seoMatch}%` }}
              />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-extrabold text-cgp-purple">{activeProfile.atsScore}% Score</span>
            </div>
            <div className="w-full h-2 rounded-full bg-cgp-surface overflow-hidden p-0.5 border border-cgp-purple/20">
              <div
                className="h-full rounded-full bg-gradient-to-r from-cgp-purple to-cgp-pink shadow-[0_0_12px_#8b5cf6] transition-all duration-1000 ease-out"
                style={{ width: `${activeProfile.atsScore}%` }}
              />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-extrabold text-emerald-400">+{activeProfile.keywordHigher}% Higher</span>
            </div>
            <div className="w-full h-2 rounded-full bg-cgp-surface overflow-hidden p-0.5 border border-emerald-500/20">
              <div
                className="h-full rounded-full bg-gradient-to-r from-emerald-400 to-cgp-cyan shadow-[0_0_12px_#34d399] transition-all duration-1000 ease-out"
                style={{ width: "95%" }}
              />
            </div>
          </div>
        ...
        {/* Real-time Dynamic Active Skill Tags */}
        <div
          className={`flex flex-wrap gap-1.5 pt-2 relative z-10 transition-opacity duration-300 ${
            isFading ? "opacity-30" : "opacity-100"
          }`}
        >
          {activeProfile.tags.map((tag, tIdx) => (
            <span
              key={tIdx}
